...
import { fetchPartLogDistinctValues } from '../services/api';

const FILTERABLE_FIELDS = [
  { field: 'event_type', label: 'Event Type' },
  { field: 'database', label: 'Database' },
  { field: 'table', label: 'Table' },
  { field: 'partition_id', label: 'Partition ID' },
  { field: 'merge_reason', label: 'Merge Reason' },
  { field: 'merge_algorithm', label: 'Merge Algorithm' },
];

export function PartLogFilterPanel() {
  const [isOpen, setIsOpen] = useState(false);
  const [expandedField, setExpandedField] = useState<string | null>(null);
  const [fieldValues, setFieldValues] = useState<Record<string, string[]>>({});
  const [loadingField, setLoadingField] = useState<string | null>(null);
  const { timeRange, partLogFieldFilters, setPartLogFieldFilter, clearPartLogFieldFilter, clearAllPartLogFilters } = useQueryStore();
  ...
  const loadFieldValues = async (field: string) => {
    ...
    try {
      const values = await fetchPartLogDistinctValues(field, timeRange);
  ...
  const toggleValue = (field: string, value: string) => {
    ...
    if (current.includes(value)) {
      setPartLogFieldFilter(field, current.filter((v) => v !== value));
    } else {
          ...
          <div className="absolute left-0 top-full mt-1 bg-gray-800 border border-gray-600 rounded shadow-lg z-50 min-w-[280px]">
            <div className="flex items-center justify-between p-2 border-b border-gray-700">
              ...
              <div className="flex items-center gap-2">
                {activeFilterCount > 0 && (
                  <button
                    onClick={clearAllPartLogFilters}
            ...
            <div className="max-h-96 overflow-y-auto">
              {FILTERABLE_FIELDS.map(({ field, label }) => {
                const selectedValues = partLogFieldFilters[field] || [];
                  ...
                  <div key={field} className="border-b border-gray-700/50 last:border-0">
                    ...
                    {isExpanded && (
                      <div className="px-2 pb-2">
                        ...
                        {selectedValues.length > 0 && (
                          <button
                            onClick={() => clearPartLogFieldFilter(field)}
